defmodule SbWeb.PromotionController do
  ...
  def index(conn, _params) do
    ...
    case _params["state"] do
      val -> promotions = Cases.list_promotions_by_state(String.to_existing_atom(val))
             render(conn, "index.json", promotions: promotions)
      nil -> promotions = Cases.list_promotions()
             render(conn, "index.json", promotions: promotions)
    end
  ...
  def create(conn, %{"promotion" => promotion_params}) do
    ...
    case Cases.get_event!(promotion_params["event_id"]) do
      event ->
        ...
        promotion_params2 = Map.merge(promotion_params,%{"code" => code_gen, "expiration" => expiration_gen, "state" => true})

defmodule SbWeb.RideController do
  ...
  @doc """
  POST a ride. /v1/rides
  validate expiration of promotion code
  validate range with poligon
  if valid return poliline with route

  Raises `HTTP CODES` when validate.
    ...
    case Cases.get_promotion_by_code!(code) do
      promotion ->
        ...
        if diff < 0 do
          ...
          # validate range
          in_range_ori =
            close_to(0, 3, ride_params["origin_lat"], ride_params["origin_lng"], points)
          ...
          if !in_range_ori && !in_range_dest do
            ...
            # get route from google map
            case GoogleMaps.directions(
                   "#{Float.to_string(ride_params["origin_lat"])},#{
                     Float.to_string(ride_params["origin_lng"])
                   }",
                   "#{Float.to_string(ride_params["dest_lat"])},#{
                     Float.to_string(ride_params["dest_lng"])
                   }",
                   key: "AIzaSyAGi2DzC-UBYivHrYwejfe0CNCJOmhukxM"
                 ) do
              {:ok, result} ->
                params = %{
                  ammount: promotion.ammount,
                  location_initial_lat: ride_params["origin_lat"],
                  location_initial_lng: ride_params["origin_lng"],
                  location_final_lat: ride_params["dest_lat"],
                  location_final_lng: ride_params["dest_lng"],
                  when: current,
  ...
  def close_to(n, j, pos_lat, pos_lon, points) do
    if Enum.at(Enum.at(points, n), 0) > pos_lat != Enum.at(Enum.at(points, j), 1) > pos_lat and
         pos_lon <
           (Enum.at(Enum.at(points, j), 1) - Enum.at(Enum.at(points, n), 1)) *
             (pos_lat - Enum.at(Enum.at(points, n), 0)) /
             (Enum.at(Enum.at(points, j), 0) - Enum.at(Enum.at(points, n), 0)) +
             Enum.at(Enum.at(points, n), 1) do
      true
    else
